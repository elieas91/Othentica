...
const SecurityCardsManager = () => {
  ...
  const fetchCards = useCallback(async () => {
    try {
      ...
      if (response.success) {
        ...
        await showErrorAlert('Error', response.message || 'Failed to load security cards');
      }
    } catch (err) {
      await showErrorAlert('Error', 'Failed to load security cards: ' + err.message);
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleCreate = async (data) => {
    try {
      ...
      const response = await apiService.createSecurityFeatureAdmin(submitData);
      
      if (response.success) {
        await showSuccessAlert('Success!', 'Security card created successfully!');
        await fetchCards();
        setShowForm(false);
        setFormData({});
        setImagePreview(null);
      } else {
        await showErrorAlert('Error', response.message || 'Failed to create security card');
      }
    } catch (err) {
      await showErrorAlert('Error', 'Failed to create security card: ' + err.message);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleUpdate = async (data) => {
    try {
      ...
      const response = await apiService.updateSecurityFeatureAdmin(editingCard.id, submitData);
      
      if (response.success) {
        await showSuccessAlert('Success!', 'Security card updated successfully!');
        await fetchCards();
        setEditingCard(null);
        setFormData({});
        setImagePreview(null);
        ...
        await showErrorAlert('Error', response.message || 'Failed to update security card');
      }
    } catch (err) {
      await showErrorAlert('Error', 'Failed to update security card: ' + err.message);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (response.success) {
          await showSuccessAlert('Deleted!', 'The security card has been deleted successfully.');
          await fetchCards();
        } else {
          await showErrorAlert('Error', response.message || 'Failed to delete security card');
        }
      } catch (err) {
        await showErrorAlert('Error', 'Failed to delete security card: ' + err.message);
      } finally {
        setIsLoading(false);
      }
    }
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-600">Manage security feature cards with images and descriptions</p>
        </div>
        <button
      ...
      {isLoading && (
        <div className="flex items-center justify-center py-12">
          ...
          <span className="ml-2 text-gray-600">Loading security cards...</span>
        </div>
      )}
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-2xl font-bold text-primary font-poppins">
                  {editingCard ? 'Edit Security Card' : 'Add New Security Card'}
                </h3>
                <p className="text-gray-600 mt-1">
                  {editingCard ? 'Update card details' : 'Create a new security feature card'}
                </p>
              </div>
              <button
                onClick={closeModal}
...
export default SecurityCardsManager;
