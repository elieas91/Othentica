...
const MeetTheFounders = () => {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const appointmentData = hibaCalendarService.formatAppointmentData(
        selectedDate,
        selectedTime
      );

      // Validate the appointment time
      const validation = hibaCalendarService.validateAppointmentTime(
            ...
            >
              ...
              <div
                className={`w-full md:w-1/2 ${
                  member.flipped ? 'order-1' : 'order-2'
                }`}
              >
                <div className="relative h-fit overflow-visible rounded-lg flex justify-center items-center">
                  ...
                  >
                    ...
                    {index === 0 && (
                      <button
                        onClick={openModal}
                        className="w-28 md:w-36 h-10 md:h-12 bg-yellow-400 text-white rounded-lg flex items-center justify-start cursor-pointer hover:scale-105 transition-transform shadow-lg px-2 gap-x-1 md:gap-x-2"
