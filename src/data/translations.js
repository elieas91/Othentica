/**
 * Public site static strings in EN and AR.
 * Use with PublicLocaleContext: t(key) returns the string for the current locale.
 */
export const translations = {
  en: {
    // Features & Benefits
    features: 'Features',
    benefits: 'Benefits',
    loadingFeatures: 'Loading features...',
    loadingBenefits: 'Loading benefits...',

    // Services
    ourSolutions: 'Our Solutions',
    ourSolutionsSubtitle: 'Discover our comprehensive approach to Corporate Health that nurtures every aspect of your being.',

    // Clients
    ourClients: 'Our Clients',
    ourClientsSubtitle: 'Trusted by leading companies across various industries',
    loadingClients: 'Loading clients...',

    // Security
    enterpriseSecurity: 'Enterprise-Grade Security and Scale',
    enterpriseSecuritySubtitle: 'Our platform is built to protect your organization at every layer, combining advanced security with seamless scalability.',
    loadingSecurity: 'Loading security features...',
    errorLoadingSecurity: 'Error loading security features:',
    usingFallbackData: 'Using fallback data...',

    // Mobile Experience
    mobileExperience: 'Mobile Experience',
    mobileExperienceSubtitle: 'Experience our innovative mobile solutions that transform ideas into exceptional digital experiences',
    mobileAppShowcase: 'Mobile App Showcase',
    bookADemo: 'Book a Demo',
    getItOnGooglePlay: 'GET IT ON Google Play',
    downloadOnAppStore: 'Download on the App Store',
    downloadOthenticaApp: 'Download Othentica App',
    downloadCardFallback: 'Experience seamless access to our services, exclusive offers, and the latest updates, all in one place. The Othentica app is available on both the App Store and Google Play. Get it to begin your journey today!',
    noImagesAvailable: 'No images available',

    // About page
    aboutOthentica: 'About Othentica',
    craftingCorporateHealth: 'Crafting Corporate Health',
    missionVisionValues: 'Mission Vision Values',
    mission: 'Mission',
    vision: 'Vision',
    values: 'Values',
    loadingAbout: 'Loading about page...',

    // Meet the Founders (CEO / COO)
    meetTheFounders: 'Meet the Founders',
    messageFromCEO: 'A Message from Our CEO',
    messageFromCOO: 'A Message from our COO',
    coFounderCEO: 'Co-founder & CEO',
    coFounderCOO: 'Co-founder & COO',

    // Dashboard - Services Management
    servicesManagement: 'Services Management',
    servicesManagementSubtitle: 'Manage services and solutions for the website',
    addService: 'Add Service',
    loadingServices: 'Loading services...',
    untitledService: 'Untitled Service',
    noDescriptionProvided: 'No description provided',

    // Testimonials
    testimonials: 'Testimonials',
    addYourTestimonial: 'Add your Testimonial',
    videoTestimonials: 'Video Testimonials',
    videoTestimonialsSubtitle: 'Hear from our clients in their own words.',
    videoTestimonialsWatch: 'Watch what others have to say about',
    videoTestimonialsBrand: 'Othentica',
    loadingTestimonials: 'Loading testimonials...',
    unableToLoadTestimonials: 'Unable to load testimonials. Please try again later.',
    noTestimonialsAvailable: 'No testimonials available at the moment.',

    // Generic
    loading: 'Loading...',
  },
  ar: {
    // Features & Benefits
    features: 'الميزات',
    benefits: 'الفوائد',
    loadingFeatures: 'جاري تحميل الميزات...',
    loadingBenefits: 'جاري تحميل الفوائد...',

    // Services
    ourSolutions: 'حلولنا',
    ourSolutionsSubtitle: 'اكتشف نهجنا الشامل للصحة المؤسسية الذي يعزز كل جانب من جوانب وجودك.',

    // Clients
    ourClients: 'عملاؤنا',
    ourClientsSubtitle: 'موثوق به من قبل شركات رائدة في مختلف الصناعات',
    loadingClients: 'جاري تحميل العملاء...',

    // Security
    enterpriseSecurity: 'الأمان والتوسع بمستوى المؤسسات',
    enterpriseSecuritySubtitle: 'تم بناء منصتنا لحماية مؤسستك على كل المستويات، مع الجمع بين الأمان المتقدم وقابلية التوسع السلسة.',
    loadingSecurity: 'جاري تحميل ميزات الأمان...',
    errorLoadingSecurity: 'خطأ في تحميل ميزات الأمان:',
    usingFallbackData: 'جاري استخدام البيانات الاحتياطية...',

    // Mobile Experience
    mobileExperience: 'تجربة الهاتف المحمول',
    mobileExperienceSubtitle: 'اكتشف حلولنا المبتكرة للهاتف المحمول التي تحول الأفكار إلى تجارب رقمية استثنائية',
    mobileAppShowcase: 'عرض تطبيق الهاتف',
    bookADemo: 'احجز عرضاً توضيحياً',
    getItOnGooglePlay: 'متوفر على Google Play',
    downloadOnAppStore: 'تنزيل من App Store',
    downloadOthenticaApp: 'تنزيل تطبيق Othentica',
    downloadCardFallback: 'استمتع بالوصول السلس إلى خدماتنا وعروضنا الحصرية وآخر التحديثات، كل ذلك في مكان واحد. تطبيق Othentica متاح على App Store و Google Play. احصل عليه لتبدأ رحلتك اليوم!',
    noImagesAvailable: 'لا توجد صور متاحة',

    // About page
    aboutOthentica: 'عن أوثنتيكا',
    craftingCorporateHealth: 'صياغة الصحة المؤسسية',
    missionVisionValues: 'المهمة والرؤية والقيم',
    mission: 'المهمة',
    vision: 'الرؤية',
    values: 'القيم',
    loadingAbout: 'جاري تحميل صفحة عنّا...',

    // Meet the Founders (CEO / COO)
    meetTheFounders: 'تعرف على المؤسسين',
    messageFromCEO: 'رسالة من مديرنا التنفيذي',
    messageFromCOO: 'رسالة من مدير العمليات',
    coFounderCEO: 'الشريك المؤسس والمدير التنفيذي',
    coFounderCOO: 'الشريك المؤسس ومدير العمليات',

    // Dashboard - Services Management
    servicesManagement: 'إدارة الخدمات',
    servicesManagementSubtitle: 'إدارة الخدمات والحلول للموقع',
    addService: 'إضافة خدمة',
    loadingServices: 'جاري تحميل الخدمات...',
    untitledService: 'خدمة بدون عنوان',
    noDescriptionProvided: 'لا يوجد وصف',

    // Testimonials
    testimonials: 'شهادات',
    addYourTestimonial: 'أضف شهادتك',
    videoTestimonials: 'شهادات فيديو',
    videoTestimonialsSubtitle: 'استمع إلى عملائنا بأصواتهم.',
    videoTestimonialsWatch: 'شاهد ما يقوله الآخرون عن',
    videoTestimonialsBrand: 'أوثنتيكا',
    loadingTestimonials: 'جاري تحميل الشهادات...',
    unableToLoadTestimonials: 'تعذر تحميل الشهادات. يرجى المحاولة لاحقاً.',
    noTestimonialsAvailable: 'لا توجد شهادات متاحة في الوقت الحالي.',

    // Generic
    loading: 'جاري التحميل...',
  },
};

/**
 * Returns a t(key) function for the given locale.
 * @param {string} locale - 'en' | 'ar'
 * @returns {(key: string) => string}
 */
export function getT(locale) {
  const lang = locale === 'ar' ? 'ar' : 'en';
  const dict = translations[lang];
  return (key) => dict[key] ?? translations.en[key] ?? key;
}
